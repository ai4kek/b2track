...
h_xyvertex = TH2D(
    "xyvertex", "vertex in xy;x/#mum;y/#mum", 500, -500, 500, 500, -500, 500
)
h_zxvertex = TH2D(
    "zxvertex", "vertex in zx;z/#mum;x/#mum", 500, -500, 500, 500, -500, 500
)
h_zyvertex = TH2D(
    "zyvertex", "vertex in zy;z/#mum;y/#mum", 500, -500, 500, 500, -500, 500
)
...
class ShowMCParticles(Module):
    ...
    def event(self):
        ...
        for mc in mcParticles:
            ...
            h_xyvertex.Fill(
                mc.getProductionVertex().X() * 1e4, mc.getProductionVertex().Y() * 1e4
            )
            h_zxvertex.Fill(
                mc.getProductionVertex().Z() * 1e4, mc.getProductionVertex().X() * 1e4
            )
            h_zyvertex.Fill(
                mc.getProductionVertex().Z() * 1e4, mc.getProductionVertex().Y() * 1e4
            )
