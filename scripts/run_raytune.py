...
def main():
    """Parse arguments, initialize CSV, run Ray Tune optimization, and report best parameters."""
    # Parse arguments
    parser = argparse.ArgumentParser(description="Ray Tune Optimization.")
    parser.add_argument(
    ...
    # Configure SQLite logger for persistent storage
    sqlite_callback = SQLiteLoggerCallback(
        filename="ray_results.db", table_name="trials"
    )

    # Configure sync for distributed execution
    sync_config = tune.SyncConfig(
        sync_on_checkpoint=True,
        sync_period=300,  # Sync every 5 minutes
        upload_dir="ray_results",  # Persistent storage directory
    )

    # Run optimization with fault tolerance
    print(f"\n[INFO] Starting Ray Tune optimization with {args.trials} trials")
    analysis = tune.run(
        trial_objective,
        config=search_space,
        search_alg=BasicVariantGenerator(max_concurrent=args.workers),
        scheduler=scheduler,
        ...
        checkpoint_score_attr="f1_score",  # Use F1 score for checkpointing
        checkpoint_freq=10,  # Checkpoint every 10 trials
        checkpoint_at_end=True,  # Checkpoint at end of optimization
