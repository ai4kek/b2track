...
## _Tracking Parameter Optimization_

The tracking parameters can be optimized using the grid search approach implemented in `optimize_tracking.py`. This script runs multiple iterations of tracking reconstruction with different parameter combinations and evaluates the performance based on hit efficiency and purity.

### _Key Parameters to Optimize_

The following parameters in the `ToCDCCKF` module can be optimized:

- `maximalDeltaPhi`: Maximal distance in phi between wires for Z=0 plane
- `maximalLayerJump`: Maximal jump over N layers
- `minimalPtRequirement`: Minimal Pt requirement for the input tracks
- `pathMaximalCandidatesInFlight`: Maximum number of candidates in flight
- `stateMaximalHitCandidates`: Maximum number of hit candidates per state

### _Optimization Workflow_

1. Run the optimization script:
   ```bash
   python3 optimize_tracking.py --input mixed_sim.root --output_dir optimization_results
   ```

2. The script will:
   - Create multiple parameter combinations
   - Run tracking reconstruction for each combination
   - Analyze the results using tracking_performance.py and tracks.py
   - Calculate efficiency and purity metrics
   - Find the optimal parameter values

3. Results are saved to:
   - `optimization_results/optimization_results.csv`: All parameter combinations and their scores
   - `optimization_results/best_parameters.txt`: The best parameter values
   - `optimization_results/optimization_results.pdf`: Visualization of the results
...
The tracking performance analysis workflow consists of three main scripts:

1. `main_rec.py`: Runs tracking reconstruction and outputs a ROOT file
2. `tracking_performance.py`: Processes the ROOT file to create ntuples with tracking metrics
3. `tracks.py`: Analyzes the ntuples to calculate tracking efficiency and purity